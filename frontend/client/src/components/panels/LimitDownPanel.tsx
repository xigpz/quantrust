/**
 * LimitDownPanel - 跌停监控面板
 * Design: 暗夜终端 - 跌停股监控
 */
import { useState } from 'react';
import { useLimitDown, formatPrice, formatPercent, formatNumber } from '@/hooks/useMarketData';
import { BarChart3, RefreshCw } from 'lucide-react';
import { useStockClick } from '@/pages/Dashboard';
import { getChangeColor } from '@/hooks/useMarketData';

type SortKey = 'name' | 'price' | 'change_pct' | 'turnover' | 'turnover_rate' | 'timestamp';
type SortDir = 'asc' | 'desc';

export default function LimitDownPanel() {
  const { data: stocks, loading, refetch } = useLimitDown();
    ...
    <div className="flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border sticky top-0 bg-card z-10">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">跌停监控</h2>
      ...
      <table className="w-full text-xs min-w-[600px]">
        ...
        <tbody>
          {sorted.map((stock) => (
            ...
            >
              <td className="py-1.5 px-3">
                <div className="flex flex-col">
                  <span className="font-medium text-down">{stock.name}</span>
              ...
              <td className="text-right py-1.5 px-2 font-mono-data font-medium text-down">
          ...
          {(!stocks || stocks.length === 0) && !loading && (
            <tr>
              <td colSpan={5} className="text-center py-8 text-muted-foreground">暂无跌停股</td>
            </tr>
          )}
          {loading && (
            <tr>
